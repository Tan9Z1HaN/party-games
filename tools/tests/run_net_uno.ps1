# Plays a whole UNO game across real processes on loopback.
#
...
#
# Usage:
#   .\tools\tests\run_net_uno.ps1
#   .\tools\tests\run_net_uno.ps1 -Clients 3
...
$script = 'res://tools/tests/net_uno.gd'
$logDir = Join-Path ([System.IO.Path]::GetTempPath()) 'party-games-net-uno'
...
Write-Host "Host + $Clients client(s) playing a full UNO game on loopback" -ForegroundColor Cyan

$hostProc = Start-Uno -Tag 'host' -GodotArgs @('--role=host', '--name=host', "--expect=$Clients")
Start-Sleep -Seconds 3

$clientProcs = @()
for ($i = 1; $i -le $Clients; $i++) {
    $clientProcs += Start-Uno -Tag "client$i" -GodotArgs @('--role=client', "--name=p$i", '--ip=127.0.0.1')
    Start-Sleep -Milliseconds 900
}

$all = @($hostProc) + $clientProcs
foreach ($p in $all) {
    if (-not $p.Proc.WaitForExit(70000)) {
...
foreach ($p in $all) {
    ...
    if ($text -notmatch '_HAND=') { $failed += "$($p.Tag)(no hand)" }
...
Write-Host 'UNO net round passed.' -ForegroundColor Green
exit 0
