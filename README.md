...
桌面上窗口是 540×960：`project.godot` 里设了 `window_width_override` / `window_height_override`，
否则 1080×1920 的视口在多数显示器上开出来的窗口比屏幕还高。这两个键只在桌面平台生效，手机导出时会被忽略。

> **改 `project.godot` 之前先关掉编辑器**，并且**不要**在设置项之间写注释。
> 编辑器保存时会把这些注释和后面那个键名合并成一条乱码项，顺手把真正的设置删掉——
> 实测丢过 `window_width_override` 和 `rendering_method.mobile`。
> 要写说明就写在这里。
